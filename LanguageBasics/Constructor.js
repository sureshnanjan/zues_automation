...
describe('Testing constructor functions', () => {
    describe('Testing constructor function Category', () => {
        let marine = new Category(5, 'Marine');

        it('Category id should be same as input', () => {
            assert.equal(marine.id, 5);
        });

        it('Category name should be same as input', () => {
            assert.equal(marine.name, 'Marine');
        });

        it('The printCategory function should print category details - id and name', () => {
            assert.equal(marine.printCategory(), 'Marine - 5');
        });
    });

    describe('Testing constructor function Pet', () => {
        let turtle = new Pet(1000, 'Turtle', 'turtle.png', 'Pending');

        it('Pet ID should be same as input', () => {
            assert.equal(turtle.id, 1000);
        });

        it('Pet name should be same as input', () => {
            assert.equal(turtle.name, 'Turtle');
        });

        it('Pet photoUrl should be same as input', () => {
            assert.equal(turtle.imageUrl, 'turtle.png');
        });

        it('Pet status should be same as input', () => {
            assert.equal(turtle.status, 'Pending');
        });

        it('The printPet function should print pet details - id, name, photoUrl, status', () => {
            assert.equal(clownfish.printPet(), '1000 - PetName -  Clownfish, PhotoUrl - turtle.png, Status - Pending');
        });
    });

    describe('Testing User constructor function', () => {
        let newUser = new User(201, 'testuser3', 'Firstname3', 'Lastname3', 'testuser3@test.com', 'Password123', '3333333333', 1);
        ...
        it('The printUser function should print the id, username, firstName, lastName, email, password, phone and userStatus of the user', () => {
            assert.equal(newUser.printUser(), '201 - testuser3, Firstname3, Lastname3, testuser3@test.com, Password123, 3333333333, 1');
        });
    });
});