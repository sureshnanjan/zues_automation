...
describe('This is a test to verify function', function() {
  describe('This is for testing category constructor', function () {
    it('It should verify the category values', function() {
      var Dog = new Category(1,'Dogs');
      assert.equal(Dog.id,1);
      assert.equal(Dog.name,'Dogs');

    });
  });
  describe('This is for testing apiresponse  function', function () {
  ...
  describe('This is for testing user  function', function () {
    it('It should verify the user constructor', function() {
      var use = new User(1, 'tanmaya','Tanmay','Agrawal', 'tanmay@example.com',  'password123', '0123456789', 100);
      assert.equal(use.id,1);
      assert.equal(use.username,'tanmaya');
      assert.equal(use.firstName,'Tanmay');
      assert.equal(use.lastName,'Agrawal');
      assert.equal(use.email,'tanmay@example.com');
      assert.equal(use.password,'password123');
      assert.equal(use.phone,'0123456789');
      assert.equal(use.userStatus,100);
    });
  });
  describe('This is for testing tag function', function () {
    it('It should verify the tag constructor', function() {
      var taag = new Tag(101,'Rocky');
      assert.equal(taag.id,101);
      assert.equal(taag.name,'Rocky');
    });
  });
  describe('This is for testing order function', function () {
    it('It should verify the order constructor', function() {
        var Order1 = new Order("2","pet11",1,"2023-03-03","approved",true);
        assert.equal(Order1.id,2);
        assert.equal(Order1.petId,"pet11");
        assert.equal(Order1.quantity,1);
        assert.equal(Order1.shipDate,"2023-03-03");
...
// This is an array for Users
  let Users = [
    new User(1, 'tanmaya','Tanmay','Agrawal', 'tanmay@example.com',  'password123', '0123456789', 100),
    new User(2, 'Johnc','John','Cena', 'john@example.com',  'password987', '9876543210', 100)
